...
/**
 * Hardware encoder via MediaCodec. Output muxed via MuxerWrapper.
 * Handles 720p/1080p/4K; queries capability and falls back if 4K not supported.
 */
class Encoder(
    ...
    fun init(): MediaCodec {
        // Check 4K support
        if (outWidth == 3840 || outHeight == 2160) {
            val list = MediaCodecInfo::class.java // placeholder; real check in Activity via MediaCodecList
        }
        ...
        format = MediaFormat.createVideoFormat(mime, actualWidth, actualHeight).apply {
            ...
            // Optional: KEY_PROFILE/LEVEL for AVC
        }
